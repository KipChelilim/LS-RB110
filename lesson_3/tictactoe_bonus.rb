...
loop do
  ...
  prompt("Welcome to Tic-Tac-Toe! First player to five wins!")
  ...
  loop do
    ...
    winner = winning_player(moves)

    player_win_count += 1 if winner == "User"
    ...
    case winner
    when "User"
      prompt("You won that round! Player: #{player_win_count}. Computer: #{computer_win_count}.")
    when "Computer"
      prompt("The computer won that round. Player: #{player_win_count}. Computer: #{computer_win_count}.")
    when nil
